...
static void get_config_work(struct work_struct *work)
{
	...
	for (i = 0; i < MAX_STEP_CHG_ENTRIES; i++)
		pr_err_ratelimited("step-chg-cfg: %duV(SoC) ~ %duV(SoC), %duA\n",
	...
	for (i = 0; i < MAX_STEP_CHG_ENTRIES; i++)
		pr_err_ratelimited("jeita-fcc-cfg: %ddecidegree ~ %ddecidegre, %duA\n",
	...
	for (i = 0; i < MAX_STEP_CHG_ENTRIES; i++)
		pr_err_ratelimited("jeita-fv-cfg: %ddecidegree ~ %ddecidegre, %duV\n",
